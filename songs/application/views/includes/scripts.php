                              ...
                              <div class="timeline-content mb-10 mt-n1">\
                                <div class="pe-3 mb-5">          \
                                  ...
                                  <div class="d-flex align-items-center mt-1 fs-6">             \
                                    ...
                                    <div class="d-flex align-items-center mt-1 fs-6">             \
                                      <div class="text-muted me-2 fs-7">Added '+item.created+' by</div>             \
                                      <a href="#" class="text-primary fw-bolder me-1">'+item.name+'</a>\
                                    </div>\
                                    <button class="btn btn-success" id="kt_explore_setlist">View Setlist</button>\
...
var songs_table = $('#user_songs_table').DataTable({
     dom: 'Bfrtip',
     buttons: [
        'copy', 'excel', 'pdf'
    ],
      ...
      "ajax": {
        ...
        "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
            ...
            'columns': [
            {
                data: null,
                render: function (data, type, row) {
                    return '<div class="form-check form-check-sm form-check-custom form-check-solid"><input class="form-check-input" name="multi-song" type="checkbox"  value="'+row.song_id+'" /></div>';
                }
            },
            {
                // data: "song_title",
...
$('#kt_explore_shuffled_songs').on('click', function(e) {
    ...
    $.ajax({
            type: "POST",
            url: base_url + "Songs/shuffle_songs",
            data: data,
            cache: false,
            dataType: 'json',  
            success: function(data){
...
$('#update_song_form').on('submit', function(e) {
    ...
    $.ajax({
            type: "POST",
            url: base_url + "Songs/update_song",
            data: $(this).serialize(),
            cache: false,
            dataType: 'json', 
            beforeSend : function(){
    ...
    // 
      ...
      var selectedValues = [];
        ...
        if (selectedValues.length < 1) {
            ...
            delete_item(selectedValues,url='songs/delete');
        }
